...
class deafult_settings():
    def __init__(self):
        ...
        self.cond_inp_type = 'canny'  # str: the type of the input condition image, take depth T2I as example, the input can be raw image, '
        ...
        self.adapter_ckpt = '/export/data/vislearn/rother_subgroup/feiden/models/pretrained/T2I_Adapter/t2iadapter_canny_sd15v2.pth'
                             # str: Adapter ckpt
        ...
        self.which_cond = 'canny'  #str:  sketch keypose seg depth canny style color openpose
...
def create_sd_sample_set(
    ...
    while idx < min(len(ds), n_samples):
        ...
        if control_mode == 'canny':
            hints = (np.clip(ds[idx]['hint'], 0., 1.) * 255).astype(np.uint8)
        ...
        with torch.inference_mode(), \
            ...
            hints_orgIM = (((np.clip(ds[idx]['image'], -1., 1.)/2.) + 0.5)*255.)[:, :, ::-1].astype(np.uint8)
            if opt.cond_inp_type == 'image':
                cond_model = get_cond_model(opt, getattr(ExtraCondition, which_cond))
            
            if opt.which_cond == 'canny':
                canny = cv2.cvtColor(hints_orgIM, cv2.COLOR_RGB2BGR)
                ...
                canny = cv2.Canny(canny, ds.canny_tresholds[idx][0], ds.canny_tresholds[idx][1])[..., None]
                canny = img2tensor(canny).unsqueeze(0) / 255.
                cond = canny.to(opt.device)
            else: 
                cond = process_cond_module(opt, hints, opt.cond_inp_type, cond_model)
            
            # if opt.cond_inp_type == 'canny':
            #     cond = torch.round(cond)

            # cv2.imwrite(os.path.join(sample_save_path, f'{idx:06}_depht.jpg'), tensor2img(cond))
